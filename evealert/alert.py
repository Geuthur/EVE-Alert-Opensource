...
class Alert_Agent:
    ...
    def run_socket(self):
        ...
        server_socket.bind(('localhost', 11220))

        # Setzen Sie eine Zeitüberschreitung für den Server-Socket
        server_socket.settimeout(5.0)  # Zeit in Sekunden
    ...
    def load_settings(self):
        ...
        if self.change:
            ...
            if settings:
                self.x1 = int(settings.get("alert_region_1", {}).get("x", "default_value"))
                self.y1 = int(settings.get("alert_region_1", {}).get("y", "default_value"))
                self.x2 = int(settings.get("alert_region_2", {}).get("x", "default_value"))
                self.y2 = int(settings.get("alert_region_2", {}).get("y", "default_value"))
                self.x1_faction = int(settings.get("faction_region_1", {}).get("x", "default_value"))
                self.y1_faction = int(settings.get("faction_region_1", {}).get("y", "default_value"))
                self.x2_faction = int(settings.get("faction_region_2", {}).get("x", "default_value"))
                self.y2_faction = int(settings.get("faction_region_2", {}).get("y", "default_value"))
                self.detection = settings.get("detectionscale", {}).get("value", None)
                self.mode = settings.get("detection_mode", {}).get("value", "picture")
                self.cooldowntimer = settings.get("cooldown_timer", {}).get("value", 60)
    ...
    def play_alarm_sound(self, sound):
        ...
        def play_sound():
            ...
            try:
                ...
                self.system_label.configure(text="System: ❎ Something went wrong.", text_color="red")
                wf.close()
                return
    ...
    def alarm_timer_thread(self):
        ...
        while not self.stopped:
            ...
            if self.alarm_counter >= self.alarm_frequency:
                ...
                self.timer = time.sleep(int(self.cooldowntimer))  # Warten Sie 1 Minute (60 Sekunden)
